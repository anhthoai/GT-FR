var Search = function () {
    ...
    return {
        ...
        init: function () {
			...
			document.getElementById("btn_search_prev").disabled = true;
			document.getElementById("btn_search_next").disabled = true;
			...
			var history_search_result = eval("");
			var history_search_id = -1;
			var history_index = -1;
			...
			$("#close_person").on('mousedown', function () {
				$('#RegisterPerson').modal('hide');
			});
			
			$("#save_person").on('mousedown', function () {
				
				var name = document.getElementById('register_person-name').value.trim();
				var sex = document.getElementById('register_person-sex').value.trim();
				var birthday = document.getElementById('register_person-birthday').value.trim();
				var home = document.getElementById('register_person-home').value.trim();
				var email = document.getElementById('register_person-email').value.trim();
				var phone = document.getElementById('register_person-phone').value.trim();
				var city = document.getElementById('register_person-city_town').value.trim();
				var country = document.getElementById('register_person-country').value.trim();
...
//				}
				...
				if (input_key!="") input_key+="&";
				input_key+="file_id="+ "/personimgs/"+history_search_result.searchlists[history_index].fileid+"_detected.jpg";
				
				if (input_key!="") input_key+="&";
				input_key+="adminid="+ history_search_result.searchlists[history_index].adminid;
				
				$.ajax({
					url: url ="RegisterAlarmPerson?userid="+userid+"&"+input_key,
					type: 'POST',
					datatype: 'json',
					async: true,
					success: function (data) {
						...
						if (obj.registerlists=="ok"){
							$("#alert-body").text("Successfully registered person data on the server!!!");
							$('#alert').modal('show');
							
							document.getElementById('register_person-name').value= "";
							document.getElementById('register_person-birthday').value="";
							document.getElementById('register_person-home').value="";
							document.getElementById('register_person-email').value="";
							document.getElementById('register_person-phone').value="";
							document.getElementById('register_person-city_town').value="";
							document.getElementById('register_person-country').value="";
							document.getElementById('register_person-sex').value="";
			...
			$("#btn_search_prev").click(function() {
				document.getElementById("btn_search_next").disabled = false;
			...
			$("#btn_search_next").click(function() {
				current_page_index = current_page_index + 1;
				document.getElementById("btn_search_prev").disabled = false;
				if (current_page_index >= total_page_count){
					current_page_index = total_page_count - 1;
					document.getElementById("btn_search_next").disabled = true;
				}
			});
			...
			$("form#search").submit(function(){
				...
				var rows_per_page = parseInt(document.getElementById('page_rows').value.trim());
				
				var start_index = current_page_index*rows_per_page;
				var end_index = (current_page_index + 1)*rows_per_page - 1;
				
				if (start_index > end_index)
				{
					start_index = end_index - rows_per_page + 1;
				}
				
				if (input_key!="") input_key+="&";
				input_key+="start_index="+start_index;
				if (input_key!="") input_key+="&";
				input_key+="end_index="+end_index;
				
				$.ajax({
					url: url ="AlarmSearch?userid="+userid+"&"+input_key+ "&mobile=0&searchId=" + searchId,
					type: 'POST',
					datatype: 'json',
					async: true,
					success: function (data) {
						...
						history_search_result = eval ("(" + data + ")");
						...
						if (obj.searchlists!="none"){
							var tb_body="";
							total_page_count = obj.searchlists[0].record_count / parseInt(document.getElementById('page_rows').value.trim());
							total_page_count = Math.floor(total_page_count) + 1;
							if (current_page_index >= total_page_count){
								current_page_index = total_page_count - 1;
								document.getElementById("btn_search_next").disabled = true;
							}
							...
							for (var i=0;i<obj.searchlists.length;i++){
								...
								if (score == 0)
								{
									...
									+'<td>'+img_field2+'</td>'
									+'<td>'+img_field3+'</td>'
									+'<td class="view_td btn default btn-xs blue-stripe" id="view_'+id+'">'+viewstr+'</td>'
							...
							$('.edit_td').on('mousedown', function () {
								...
								var id = sep_arr[1];
								history_search_id = id;
								history_index = -1;
								for (var i=0;i<history_search_result.searchlists.length;i++)
								{
									if (id == history_search_result.searchlists[i].id)
									{
										history_index = i;
										break;
									}
								}
								$("#register_person_photo").attr("src","personimgs/"+history_search_result.searchlists[history_index].fileid+"_detected.jpg");
								$('#RegisterPerson').modal('show');
							});	
							$('.view_td').on('mousedown', function () {
								...
								$("#person_img").attr("src","personimgs/"+history_search_result.searchlists[history_index].fileid+"_full.jpg");
