var Search = function () {
    ...
    return {
        ...
        init: function () {
			...
			var user_search_result = eval("");
			var update_user_id = -1;
			var sel_edit_user_photo = 0;
			...
			$("form#search").submit(function(){
				...
				var id = document.getElementById('id1').value.trim();
...
//				if (name==''&&country==''&&city==''&&address==''&&email==''&&sel_photo==false){
...
//				}		
				...
				if (input_key!="") input_key+="&";
				input_key+="admin_id="+adminid;
				
				if (input_key!="") input_key+="&";
				input_key+="member="+member;
				
				$.ajax({
					url: url ="UserSearch?userid="+userid+"&"+input_key+ "&mobile=0&searchId=" + searchId,
					type: 'POST',
					datatype: 'json',
					async: true,
					success: function (data) {
...
//							sel_photo=0;
						...
						var obj = eval ("(" + data + ")");
						user_search_result = eval ("(" + data + ")");
						if (obj.searchlists=="none"){
							$("#alert-body").text("There is no result!!!");
							$('#alert').modal('show');
							
						}
						else if (obj.searchlists=="deny"){
							$("#alert-body").text("You are not admin!");
							$('#alert').modal('show');
						...
						if (obj.searchlists!="none"){
							...
							for (var i=0;i<obj.searchlists.length;i++){
								...
								//var	video_field = '<video class="res_face" src="rtsp://admin:admin@115.73.214.170:554">' + '/video>';
							...
							$('.edit_td').on('mousedown', function () {
								...
								update_user_id = id;
								var index = -1;
								for (var i=0;i<user_search_result.searchlists.length;i++)
								{
									if (id == user_search_result.searchlists[i].id)
									{
										index = i;
										break;
									}
								...
								var name = user_search_result.searchlists[i].fullname;
								if (name==null || name=="null") name = " ";
								var email = user_search_result.searchlists[i].email;
								if (email==null || email=="null") email = " ";
								var address = user_search_result.searchlists[i].address;
								if (address==null || address=="null") address = " ";
								var city= user_search_result.searchlists[i].city;
								if (city==null || city=="null") city = " ";
								var country = user_search_result.searchlists[i].country;
								if (country==null || country =="null") country = " ";
								var user_id = user_search_result.searchlists[i].user_id;
								if (user_id==null || user_id=="null") user_id = " ";
								
								document.getElementById('edit_user_fullname').value= name;
								document.getElementById('edit_user_email').value=email;
								document.getElementById('edit_user_address').value=address;
								document.getElementById('edit_user_city_town').value=city;
								document.getElementById('edit_user_country').value=country;
							...
							$('.delete_td').on('mousedown', function () {
								...
								if (result) {
									$.ajax({
										url: url ="UserDelete?userid="+userid+"&index="+id,
										type: 'POST',
										datatype: 'json',
										async: true,
										success: function (data) {
											...
											var obj = eval ("(" + data + ")");
											if (obj.userdelete=="fail"){
												$("#alert-body").text("Failed to delete user!");
												$('#alert').modal('show');
											}
											else if (obj.userdelete=="ok")
											{
												$("#alert-body").text("Successfully deleted user!");
												$('#alert').modal('show');
												$('#tr_'+id+'').hide();
											}
										},
