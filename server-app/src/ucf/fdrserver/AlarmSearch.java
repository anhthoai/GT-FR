 ...
 */
@WebServlet("/AlarmSearch")
public class AlarmSearch extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(from!=null){
			if(sql!="") sql+=" AND access>='"+from.trim()+" 00:00:00'";
			else sql="access>='"+from.trim() + " 00:00:00'";
		}
		
		String to=(String) request.getParameter("to");
		if(to!=null){
			if(sql!="") sql+=" AND access<'"+to.trim()+" 23:59:59'";
			else sql="access<'"+to.trim() + " 23:59:59'";
		}
		...
		try {
				...
				while (rs.next() == true) {
					...
					for (int ii = 1; ii <= columns; ++ii) {
						if (ii == 3)
						{
							row[ii - 1] = rs.getTimestamp(ii);
						}
						else if (ii == 8)
						{
							row[ii - 1] = rs.getDate(ii);
						}
						else
						{
							row[ii - 1] = rs.getString(ii);
						}
					}
					rows1.add(row);
				}
				ArrayList<Object[]> rows = new ArrayList<Object[]>();
				for (int i = rows1.size() - 1; i >=0 ; i--)
				{
				...
				{
					int row_per_page = end_index - start_index + 1;
					if (end_index > rows.size() - 1) 
					{
						end_index = rows.size() - 1;
						start_index = start_index;//end_index - row_per_page + 1;
					}else
					{
					...
					for (int i = start_index; i <= end_index; i++) {
						id = Integer.parseInt(rows.get(i)[0].toString());
						int score = Integer.parseInt(rows.get(i)[4].toString());
						name = rows.get(i)[5].toString();
						
						if (rows.get(i)[6] == null) sex = "";
						else sex =rows.get(i)[6].toString(); 
						
						if (rows.get(i)[7]==null) birth="";
						else {
							Date birth_dt=(Date)rows.get(i)[7];
							birth=formatter.format(birth_dt);
						}
							
						if (rows.get(i)[8]==null) home="";
						else home=rows.get(i)[8].toString(); 
						
						email=rows.get(i)[11].toString();
						phone=rows.get(i)[12].toString();
						
						if (rows.get(i)[9]==null) city_str="";
						else city_str=rows.get(i)[9].toString();
						
						if (rows.get(i)[10]==null) country_str="";
						else country_str=rows.get(i)[10].toString();
						String alarm_id=rows.get(i)[1].toString();
						adminid=rows.get(i)[13].toString();
						
						if (rows.get(i)[14] == null)
							group_name = "Nogroup";
						else
							group_name=rows.get(i)[14].toString();
						
						Date d1 = null;
						//String time = new   SimpleDateFormat("HH-mm-ss.SSS").format(rs.getTime("access").getTime());

						Timestamp timestamp = (Timestamp)rows.get(i)[2];
						...
						String access_date = formatter.format(d1);
						...
						json_str+="record_count : "+rows.size()+",";
